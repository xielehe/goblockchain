...
func (cli *CommandLine) printChain() {
	...
	for {
		...
		for _, v := range block.Transactions {
			fmt.Printf("__txId: %x\n", v.ID)
			fmt.Println("__Input:")
			for _, inv := range v.Inputs {
				fmt.Printf("____inputId: %x\n", inv.ID)
				fmt.Printf("____inputOut: %v\n", inv.Out)
				fmt.Printf("____inputSig: %v\n", inv.Sig)
			}
			fmt.Println("__Output:")
			for _, outv := range v.Outputs {
				fmt.Printf("____outputValue: %v\n", outv.Value)
				fmt.Printf("____outputPubKey: %v\n", outv.PubKey)
			}
		}
